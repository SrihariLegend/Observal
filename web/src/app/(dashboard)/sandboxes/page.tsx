...
export default function SandboxesPage() {
  const { data, isLoading } = useRegistryList("sandboxes");
  const router = useRouter();

  return (
    <DashboardShell>
      <PageHeader title="Sandboxes" breadcrumbs={[{ label: "Home", href: "/" }, { label: "Sandboxes" }]} />
      <RegistryTable
        data={(data as Row[]) ?? []}
        columns={columns}
        isLoading={isLoading}
        onRowClick={(row) => router.push(`/sandboxes/${row.id}`)}
      />
    </DashboardShell>
  );
}
