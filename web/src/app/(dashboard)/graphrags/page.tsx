...
const columns: ColumnDef<Row, unknown>[] = [
  ...
  { accessorKey: "query_interface", header: "Query Interface" },
...
export default function GraphRagsPage() {
  const { data, isLoading } = useRegistryList("graphrags");
  const router = useRouter();

  return (
    <DashboardShell>
      <PageHeader title="GraphRAGs" breadcrumbs={[{ label: "Home", href: "/" }, { label: "GraphRAGs" }]} />
      <RegistryTable
        data={(data as Row[]) ?? []}
        columns={columns}
        isLoading={isLoading}
        onRowClick={(row) => router.push(`/graphrags/${row.id}`)}
      />
    </DashboardShell>
  );
}
